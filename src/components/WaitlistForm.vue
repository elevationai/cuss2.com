...
const timeline = ref('6mo');
const submitted = ref(false);

function toggleProduct(p) {
  const i = products.value.indexOf(p);
  if (i >= 0) products.value.splice(i, 1);
  ...
  <section v-if="!submitted" style="padding-top:20px;">
    <div class="wrap" style="max-width:880px;">
      <form @submit="submit" class="card" style="padding:40px;">
        ...
        <div style="margin-top:10px; color:var(--ink-2); font-size:14px;">Pick every solution that's interesting — even loosely.</div>
